...
温度、湿度、気圧、照度も取るなら、M5StickS3 の Grove 端子は I2C バスとして使う方がよい。

```text
M5StickS3 Grove
GND / 5V / G9(SDA) / G10(SCL)
  |
Grove Hub または PaHUB
  |- ENV III        温度・湿度・気圧
  |- DLight         照度
  |- Unit ADC       SEN0308土壌水分のアナログ値
  `- EXT.IO2        MOSFET INをON/OFF
```

この場合、G9を土壌水分ADC、G10をMOSFET INとして直接使う案はやめる。G9/G10をI2CのSDA/SCLに使い、土壌水分はUnit ADC、ポンプ制御はEXT.IO2へ逃がす。

### 追加センサーの置き方

- ENV III はM5箱の中に入れると箱内温湿度になる。外気を測るなら小型百葉箱、自然通風シェルター、放射シールドの中に置く。
- DLight は照度を見るため、空が見える向きに置く。ただし防水ではないため、透明カバーや小型センサーシェルターで雨を避ける。
- どちらもGroveコネクタ部はM5防滴箱内、またはセンサーシェルター内に逃がす。
- 「雨よけ下」は既存設備のことではなく、追加するセンサー用保護カバーのこと。
