...
| 状態 | 区分 | 候補 | 数量 | メモ |
|---|---|---|---:|---|
| ✓ 購入済み | 100V安全 | HATAYA BFX-013KC2 相当 | 1 | Amazon注文: 6,286円。防雨形コンセントから取る最初の延長コード |
| ✓ 購入済み | 100V保護箱 | Hrensaw 大型屋外電気ボックス IP54 / 32 x 22 x 13cm級 | 1 | Amazon注文: 3,799円。延長コードのメス側とUSB-C ACアダプタを入れる |
| ✓ 購入済み | AC to 5V | UGREEN 30W GaN 2C1A | 1 | Amazon注文: 1,584円。USB-C 5V電源 |
| ✓ 購入済み | USB-C配線 | UGREEN USB-C 100W/5A 1m | 1 | Amazon注文: 788円。簡易ウェザーステーション用にまず1本 |
| 未購入 | M5防滴箱 | 推奨: 150 x 200 x 100mm級 / 最小: BCAP112107G | 1 | 初心者は配線余裕のある150 x 200 x 100mm級を優先 |
| ✓ 購入済み | 通線部品 | オーム OA-WH22-06/13 相当 | 1 | Amazon注文: 1,692円。USB-CやGrove系ケーブルの通線 |
| 代替候補あり | 制御本体 | M5StickS3 / 手元M5Stack Core2 v1.1 | 1 | Core2はUSB-C + Grove Port Aで代替可。ケース寸法に注意 |
| 未購入 | 5V/Grove補助 | M5Stack TypeC2Grove | 1 | Grove系の5V補助 |
| 未購入 | 防水土壌水分 | DFRobot SEN0308 相当 | 1 | 土へ常設する防水センサー |
| 未購入 | 給水 | 5V-6V チューブポンプ | 1 | センサーと分離した給水 |
| 未購入 | ポンプ駆動 | DFRobot DFR0457 MOSFET 相当 | 1 | G10でポンプON/OFF |
| 未購入 | GND共通化 | WAGO 221-415 相当 | 1-2 | M5 GND、ポンプ電源-、センサーGNDをまとめる |
| 未購入 | 環境情報 簡単案 | SwitchBot 防水温湿度計 + Hub Mini/Hub 2 | 1式 | IP65屋外温湿度。データはBLE/Hub/API/Home Assistantへ逃がせる |
| 未購入 | 環境情報 照度込み | Wi-Fiウェザーステーション候補 | 1式 | 温湿度に加えて照度/日射/UVまで欲しい場合。M5とは別系統で飛ばす |
| 未購入 | 環境情報 DIY案 | ENV III + DLight + センサー保護 + I2C拡張 | 1式 | M5に全部つなぎたい場合だけ。防水処理と配線が増える |
...
| 構成 | 価格 | 温度 | 湿度 | 照度 | 雨 | ローカルAPI | クラウドAPI | 屋外 | 追加機器 | 総評 |
|---|---:|---|---|---|---|---|---|---|---|---|
| SwitchBot防水温湿度計 + Hub Mini | 7,010円 | ◎ | ◎ | × | × | △ | ○ | ◎ | Hub | 温湿度だけなら最安級 |
| M5簡易ウェザーステーション | 7,000〜10,000円 | ◎ | ◎ | ◎ | × | ◎ | × | △ | 電源箱 | 100V電源箱を除く。APIは一番素直 |
| zmart/Tuya日光・雨 + Zigbee2MQTT | 7,427円〜 | × | × | ◎ | ◎ | ◎ | × | ○ | USBドングル | 照度と雨の安い本命 |
| SwitchBot + zmart/Tuya + Zigbee2MQTT | 14,437円〜 | ◎ | ◎ | ◎ | ◎ | ○ | ○ | ○ | Hub/USBドングル | 既製品寄せの有力案 |
| Tuya/Smart Life系で統一 | 9,000〜13,000円 | ○ | ○ | ○ | ○ | △ | ○ | △ | Gateway | アプリ統一なら候補 |
| ECOWITT Wittboy GW2001/WS90 | 47,568円 | ◎ | ◎ | ◎ | ◎ | ◎ | ◎ | ◎ | Gateway | 全部入りだが高い |
...
| 部品 | 価格 | 役割 |
|---|---:|---|
| M5StickS3 | 公式 $24.90 | Wi-Fi付き本体。HTTP JSON/MQTTを直接出せる |
| ENV III/IV | 公式 $5.95〜 | 温度・湿度・気圧 |
| DLight | 公式 $5.50 | 照度 lux |
| Unit Hub / PaHUB | 公式 $4.50 | Grove分岐 |
| ダイソー透明ケース + 通線部品 | 330円 + 約1,200円 | 外装とUSB-C入口 |
| USB-C電源一式 | 約5,180円〜 | 5V常時給電。既存USB-C給電を使うなら除外 |
| 100V側屋外電源箱 | 約8,400円〜 | M5本体価格ではない。ベランダ常設の安全費用 |

USB-C ACアダプタ候補:

| 候補 | 価格 | PSE | PD | GaN | 国内メーカー | 本命度 | PSE確認根拠 |
|---|---:|---|---|---|---|---|---|
| Anker 523 Charger 47W | 3,990円 | ◎ | ◎ | ◎ | × | ◎ | 販売店ページで技術基準適合を確認。Anker公式本文ではPSE文字列なし |
| UGREEN 30W GaN 2C1A | 1,500〜2,000円級 | ◎ | ◎ | ◎ | × | ◎ | Amazon掲載名でPSE技術基準適合、PD3.0、30W、2C1A、GaNを再確認。M5はUSB-C単独利用前提 |
| オウルテック 30W PD | 3,000円前後 | ◎ | ◎ | ◎ | ◎ | ◎ | オウルテック公式にPSE適合製品と明記 |
| エレコム 30W PD | 2,000〜5,000円級 | ◎ | ◎ | × | ◎ | ◎ | エレコム公式に法令PSE 特定電気用品と明記 |

USB-Cケーブル候補:

| 候補 | 価格 | 100W | E-Marker | 高耐久 | 国内メーカー | 本命度 | 確認根拠 |
|---|---:|---|---|---|---|---|---|
| Anker USB-C 3m 100W | 1,190円〜 | ◎ | ◎ | ◎ | × | ◎ | 3mが必要な時の長尺候補 |
| UGREEN USB-C 100W 1m ナイロン | 公式1,490円 | ◎ | ◎ | ◎ | × | ◎ | 公式で100W/20V5A、eMarker、10000回以上折り曲げ、ケーブル径約6.4mmを確認。短距離なら本命 |
| Belkin BoostCharge 100W 2m | 1,300円前後〜 | ◎ | ◎ | ◎ | × | ◎ | 2mで高耐久寄りの候補 |
| オウルテック 100W 2m 高耐久 | 価格要確認 | ◎ | ◎ | ◎ | ◎ | ◎ | 国内メーカー優先の候補 |

記号: ◎=確認できた、または要件を満たす。×=未確認、対象外、または要件を満たさない。
...
| 候補 | Amazon確認 | Amazonリンク | 価格目安 | 適合ケーブル径 | 取付穴 | USB-Cコネクタ通線 | 屋外向き | 採用候補 | 見方 |
|---|---|---|---:|---|---|---|---|---|---|
| オーム OA-WH16-06/10 10個入り | ◎ | https://www.amazon.co.jp/dp/B01M8NNCXN | Chrome確認: 7,776円 / 約778円個 | 6〜10mm | 21mm | ◎ | × | × | ChromeでAmazon商品ページを確認。黒ゴム耐油型なので直射日光連続曝露は避ける扱い |
| オーム OA-WH22-06/13 5個入り | ◎ | https://www.amazon.co.jp/dp/B01M29W806 | Chromeで商品ページ確認。価格は要再確認 | 6〜13mm | 28mm | ◎ | × | × | ChromeでAmazon商品ページタイトルを確認。ただし価格欄は代替商品価格が混ざるため再確認扱い |
| オーム OA-WH16-06/10E | × | Amazon未確認 | 約740円/個級 | 6〜10mm | 21mm | ◎ | ◎ | ◎ | 耐候型なら本命。ただしAmazon直リンク未確認なのでAmazon縛りなら保留 |
| オーム OA-WH22-06/13E | × | Amazon未確認 | 約1,100円/個級 | 6〜13mm | 28mm | ◎ | ◎ | × | 耐候型だがAmazon直リンク未確認。穴が大きい |
| 汎用M20分割ケーブルグランド | ◎ | Amazon検索 | 約1,000〜2,000円/複数個 | 商品ごと | 20mm級 | × | × | × | Amazonにはあるが、USB-Cコネクタ付き通線の仕様根拠が弱い |
| ゴムグロメット + 自己融着テープ | ◎ | Amazon検索 | 約500〜1,500円 | 現物合わせ | 現物合わせ | ◎ | × | × | Amazonにはあるが、防水等級は期待せず防滴補助 |

現時点の本命は仕様上は `OA-WH16-06/10E`。ただしAmazon直リンクは未確認。Amazonだけで買うなら `OA-WH16-06/10` は存在するが、黒ゴム耐油型なので屋外常設の本命からは外す。
